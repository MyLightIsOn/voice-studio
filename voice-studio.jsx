...
// --- Inworld Voice Studio Concept ---
// A reimagined TTS playground that combines voice exploration,
// side-by-side casting, and developer-friendly UX.
        ...
        return (
          <div
            ...
            style={{
              ...
              height: animH,
              transition: isPlaying ? "height 0.1s ease" : "height 0.6s ease, opacity 0.6s ease",
            }}
          />
        );
      })}
...
// Voice card for the grid
function VoiceCard({ voice, isSelected, isInCast, onSelect, onToggleCast }) {
    ...
    >
      ...
      {isInCast && (
        <div style={{
          ...
          backgroundColor: voice.color, display: "flex",
...
// Cast comparison row
function CastRow({ voice, text, index, isPlaying, onPlay }) {
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <Waveform isPlaying={isPlaying} color={voice.color} height={36} />
      </div>
...
export default function VoiceStudio() {
  const [tab, setTab] = useState("explore"); // explore | cast | code
  const [selectedVoice, setSelectedVoice] = useState(VOICES[0]);
  const [castList, setCastList] = useState([VOICES[0], VOICES[1], VOICES[3]]);
  ...
  // Simulate play/stop
  const togglePlay = useCallback((voiceId) => {
  ...
  useEffect(() => {
    if (playingId) {
      const timer = setTimeout(() => setPlayingId(null), 4000);
  ...
  const categories = ["All", ...new Set(VOICES.map(v => v.category))];
...
const response = await fetch("https://api.inworld.ai/tts/v1/synthesize", {
  ...
  body: JSON.stringify({
    ...
    model: "inworld-tts-1.5-${model}",
    output_spec: { container: "mp3" },
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <span style={{
              marginLeft: 10, fontSize: 10, fontWeight: 600, padding: "2px 8px",
              borderRadius: 6, backgroundColor: "#6366F122", color: "#818CF8",
          ...
          <div style={{ padding: "16px 20px", borderTop: "1px solid #1E1E3F" }}>
            <button
              ...
              style={{
                ...
                background: playingId === selectedVoice.id
                  ? `linear-gradient(135deg, ${selectedVoice.color}, ${selectedVoice.color}CC)`
                  : "linear-gradient(135deg, #6366F1, #8B5CF6)",
                color: "white", border: "none", cursor: "pointer",
                fontSize: 15, fontWeight: 600, letterSpacing: "-0.01em",
                transition: "all 0.2s ease",
                animation: playingId === selectedVoice.id ? "pulseGlow 2s infinite" : "none",
              }}
            >
              {playingId === selectedVoice.id ? `⏸ Playing ${selectedVoice.name}...` : `▶ Synthesize with ${selectedVoice.name}`}
            </button>
          </div>
        </div>

        {/* RIGHT: Tabs (Explore / Cast / Code) */}
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>

          {/* Tab Bar */}
          <div style={{
            display: "flex", alignItems: "center", gap: 0,
            padding: "0 24px", borderBottom: "1px solid #1E1E3F",
          }}>
            {[
              { id: "explore", label: "Explore Voices", icon: "🎤" },
              { id: "cast", label: `Voice Casting ${castList.length ? `(${castList.length})` : ""}`, icon: "🎭" },
              { id: "code", label: "API Code", icon: "⌨️" },
            ].map(t => (
              ...
              <div style={{ display: "flex", gap: 12, marginBottom: 20, alignItems: "center" }}>
                <input
                  ...
                  style={{
                    ...
                    backgroundColor: "#13132B", color: "#E2E8F0",
                    border: "1px solid #2A2A4A", width: 200,
                    fontFamily: "inherit",
                  }}
                />
            ...
            <div style={{ flex: 1, overflow: "auto", padding: 24 }}>
              <div style={{ marginBottom: 20 }}>
                ...
                <p style={{ fontSize: 13, color: "#94A3B8", margin: 0, lineHeight: 1.6 }}>
                  Compare up to 5 voices reading the same script side-by-side. Click play on each to hear the differences, then pick your perfect cast.
                </p>
              </div>
              ...
              {castList.length === 0 ? (
                <div style={{
                  ...
                  borderRadius: 16, border: "2px dashed #2A2A4A",
                }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <div style={{
                    padding: "14px 18px", borderRadius: 12,
                    backgroundColor: "#1E1E3F", marginBottom: 8,
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: 13, color: "#94A3B8", lineHeight: 1.6 }}>
                      Try adjusting the <strong style={{ color: "#C4B5FD" }}>temperature</strong> slider on the left — lower values give consistent reads (great for agents), while higher values add dramatic flair (perfect for game characters).
                    </div>
                  </div>
                </div>
              )}
          ...
          {tab === "code" && (
            <div style={{ flex: 1, overflow: "auto", padding: 24 }}>
              <div style={{ marginBottom: 20 }}>
                ...
                <p style={{ fontSize: 13, color: "#94A3B8", margin: 0, lineHeight: 1.6 }}>
                  Copy this snippet to integrate <strong style={{ color: "#C4B5FD" }}>{selectedVoice.name}</strong> into your app. Updates live as you change voice, model, and settings.
                </p>
              </div>
              ...
              <div style={{
                ...
                {/* Code body */}
                <pre style={{
                  padding: 20, margin: 0, overflow: "auto",
                  backgroundColor: "#0B0B1E",
              ...
              <div style={{
                ...
                backgroundColor: "#13132B", border: "1px solid #2A2A4A",
              }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {[
                    ...
                    <div key={item.label} style={{
                      ...
                      padding: "8px 12px", borderRadius: 8, backgroundColor: "#1E1E3F",
                    }}>
